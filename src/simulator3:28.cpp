...
SimulationResult simulate_oscillation(
    ...
    try {
        ...
        result.probs[0][1] = std::norm(psi[3]); // \( P(\nu_e \to \nu_\mu) \)
        result.probs[0][2] = std::norm(psi[6]); // \( P(\nu_e \to \nu_\tau) \)
        /* 計算初始點的概率
        for (int j = 0; j < 3; j++) {
        ...
        */
        ...
        double t = r_start;
        // 根據問題特性選擇適當的初始步長
        double h = std::min(1.0e-7, dr / 1000.0);  // 非常保守的初始步長
